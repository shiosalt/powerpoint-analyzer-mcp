...
class TestComprehensiveMCPTools:
    ...
    @pytest.mark.asyncio
    async def test_extract_bold_text_enhanced_format(self):
        """Test extract_bold_text with enhanced response format."""
        test_suite = MCPIntegrationTestSuite()
        ...
        try:
            ...
            assert success, f"Failed to extract bold text: {response_data}"
            assert "bold_text_by_slide" in response_data
            
            # Verify enhanced format includes both old and new fields
            for slide_info in response_data["bold_text_by_slide"]:
                assert "slide_number" in slide_info
                assert "complete_text" in slide_info  # New field
                assert "bold_elements" in slide_info  # Legacy field
                assert "bold_segments" in slide_info  # New field
                
                # Verify bold segments structure
                for segment in slide_info["bold_segments"]:
                    assert "text" in segment
                    assert "start_position" in segment
