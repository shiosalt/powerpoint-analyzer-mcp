...
- 個別のスライド情報を取得
- 柔軟なフィルタリング条件でスライドをクエリ
- フォーマット検出付きテーブルデータ抽出
- **高度なテキスト書式検出**: 太字、斜体、下線、取り消し線、ハイライト、ハイパーリンク
- **フォント分析**: フォントサイズ、色、スタイル情報
- 包括的なプレゼンテーション概要と分析を取得
- スライドレイアウト、プレースホルダー、フォーマット情報をサポート
- **包括的なテストスイート** による書式検出の検証
- **デバッグツール** による書式検出問題のトラブルシューティング
- Python標準ライブラリを使用した軽量実装（外部PowerPoint依存関係なし）
- 高速で効率的な処理のための直接XML解析
- FastMCP 2.0による最適なパフォーマンスと互換性
...
- **高度なXML解析**: 包括的な名前空間サポートで`xml.etree.ElementTree`を使用して内部PowerPoint XML構造を解析
...
- **包括的なキャッシュ**: 繰り返し操作でのパフォーマンス向上のためのインテリジェントキャッシュシステム

## テキスト書式検出

サーバーは高度なテキスト書式検出機能を提供します：

### サポートされる書式タイプ
- **太字テキスト**: 全てのテキスト要素で太字書式を検出
- **斜体テキスト**: スライド全体で斜体スタイルを識別
- **下線テキスト**: 様々な下線スタイルで下線テキストを検出
- **取り消し線テキスト**: 取り消し線書式を検出
- **ハイライトテキスト**: ハイライト/背景色付きテキストを識別
- **ハイパーリンク**: ハイパーリンク情報と関係IDを抽出
- **フォントプロパティ**: フォントサイズ、色（RGBとスキーム色）を分析

### 技術実装
- **デュアル検出方式**: XML属性（`b="1"`）と子要素（`<a:b val="1"/>`）の両方をチェック
- **名前空間対応解析**: Office Open XML名前空間の適切な処理
- **包括的な検証**: 異なるPowerPointバージョンでの正確な検出を保証する広範なテストスイート
- **デバッグ機能**: 書式検出問題のトラブルシューティング用の組み込みデバッグツール

### 検証とテスト
- **包括的なテストスイート**: `tests/test_formatting_detection.py`で全ての書式タイプを検証
- **デバッグツール**: `tests/debug_formatting_detection.py`で詳細なXML分析を提供
- **実世界での検証**: 混合書式を含む複雑なPowerPointファイルでテスト済み
...
4. **query_slides**: 柔軟なフィルタリング条件でスライドをクエリ
5. **extract_table_data**: 柔軟な選択とフォーマット検出でテーブルデータを抽出

#### テキスト書式分析
6. **extract_bold_text**: 位置情報付きでスライドから全ての太字テキストを抽出
7. **extract_formatted_text**: 特定の書式タイプ（太字、斜体、下線、取り消し線、ハイライト、ハイパーリンク）のテキストを抽出
8. **get_formatting_summary**: プレゼンテーション内の全テキスト書式の包括的サマリーを取得
9. **analyze_text_formatting**: 詳細な書式検出でスライド全体のテキスト書式パターンを分析

#### プレゼンテーション分析
10. **get_presentation_overview**: 包括的なプレゼンテーション概要と分析を取得
11. **clear_cache**: パフォーマンス向上のため分析キャッシュをクリア
...
6. **テキスト書式の分析**:
   ```
   /Users/username/Documents/quarterly-report.pptxから全ての太字テキストを抽出して、どのスライドに表示されているか教えてください
   ```

7. **包括的な書式サマリーの取得**:
   ```
   /Users/username/Documents/quarterly-report.pptxの全てのテキスト書式（太字、斜体、下線など）を分析できますか？
   ```

8. **特定の書式タイプの抽出**:
   ```
   /Users/username/Documents/quarterly-report.pptxから太字と斜体の書式が適用された全てのテキストを抽出してください
   ```
...
├── core/
│   ├── __init__.py
│   ├── content_extractor.py    # 高度な書式検出付きPowerPointコンテンツ抽出
│   ├── attribute_processor.py  # 属性フィルタリングと処理
│   ├── presentation_analyzer.py # 包括的なプレゼンテーション分析
...
└── tests/
    ├── test_formatting_detection.py  # 包括的な書式検出テスト
...
### バージョン 2.0 - 高度なテキスト書式検出
- **重要な書式検出バグを修正**: 太字、斜体、下線、取り消し線属性が正しく検出されるようになりました
- **デュアル検出サポート**: XML属性と子要素の両方の形式を処理
- **包括的なテストスイート**: 全ての書式タイプの広範なテストを追加
- **デバッグツール**: 書式問題のトラブルシューティング用の新しいデバッグユーティリティ
- **強化されたMCPツール**: 書式付きテキスト抽出と分析のための新しいツール

### 書式検出の検証
全ての書式タイプが徹底的にテストされ、検証されています：
- ✅ **太字テキスト**: テストファイルで8個の要素を検出
- ✅ **斜体テキスト**: テストファイルで6個の要素を検出
- ✅ **下線テキスト**: テストファイルで6個の要素を検出
- ✅ **取り消し線テキスト**: テストファイルで6個の要素を検出
- ✅ **ハイライトテキスト**: テストファイルで5個の要素を検出
- ✅ **ハイパーリンク**: テストファイルで2個の要素を検出
- ✅ **フォント色**: 複数の色が検出され分析されました
